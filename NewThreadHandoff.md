...
- 완료: 서버 PC에서 project 상태 POST/GET, Supabase 저장, 동일 project/workstation update, 실제 `head_sha` 저장 확인
- 완료: 05-A 외부 E2E 및 05-B 읽기 전용 Git 상태 수집기 구현/테스트
- 완료: 05-C FileSystemWatcher/debounce와 project 상태 자동 전송 구현 및 테스트
- 완료: 05-C 실제 DEV PC root E2E 및 06 RS256 assertion/NAS provision E2E
- 완료: 평상시 Agent의 대용량 자동 hash/upload/reconciliation 경로 제거. `ProjectHub_Sync.ps1` 고정 manifest와 제어-plane 선반영, 별도 `ProjectHub_LargeData_Uploader.ps1`의 Supabase 기준 기존 세션 재사용, chunk resume/finalize, STAGED·CHECKPOINTED 메타데이터 경로 구현
