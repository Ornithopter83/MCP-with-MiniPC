(() => {
  ...
  const EXTENSION_BUILD = '2026-09-20.4';
  ...
  root.innerHTML = '<style></style><section class="hub-panel" aria-label="GPTWeb-Hub">'+
    ...
    '<div class="status-list">'+statusRow('web','GPT Web','—','web-connect')+statusRow('worker','Worker','—','')+statusRow('status','Status','Disconnected','')+'</div>'+
  ...
  let settings=Object.assign({},DEFAULT), currentConversationId=null, lastBoundConversationId=null, currentProjectId='', activeTaskId=null, sentTaskId=null, activeLeaseId=null, phase='IDLE', baselineAssistant='', pendingResult=null; let resetGeneration=0;
  ...
  function url(path,cfg){const c=cfg||settings;return 'http://'+c.bridgeHost+':'+c.bridgePort+c.bridgeBasePath+'/'+path;}
  ...
  function saveTaskMemory(){if(!currentConversationId||!activeTaskId)return;sessionStorage.setItem('gptweb-hub-task-'+currentConversationId+'-'+activeTaskId,JSON.stringify({sentTaskId,activeLeaseId,baselineAssistant}));} function restoreTaskMemory(taskId){if(!currentConversationId)return false;try{const value=JSON.parse(sessionStorage.getItem('gptweb-hub-task-'+currentConversationId+'-'+taskId)||'null');if(!value)return false;sentTaskId=value.sentTaskId||null;activeLeaseId=value.activeLeaseId||null;baselineAssistant=value.baselineAssistant||'';return true;}catch{return false;}} function setStatus(kind,value,tone){const e=root.querySelector('.status-row[data-kind="'+kind+'"] .status-value');if(!e)return;e.textContent=value;e.className='status-value status-'+tone;}
  ...
  function composer(){const candidates=[...document.querySelectorAll('textarea,[contenteditable="true"]')].filter(e=>visible(e)&&!e.disabled);const labeled=candidates.find(e=>{const label=[e.getAttribute('aria-label'),e.getAttribute('placeholder'),e.getAttribute('data-testid')].filter(Boolean).join(' ');return /message|메시지|prompt|질문|composer/i.test(label);});return labeled||candidates.find(e=>e.closest('main'))||candidates[0]||null;}
  function sendButton(){
    ...
    for(const scope of scopes){
      for(const selector of selectors){
        for(const button of scope.querySelectorAll(selector)){
          ...
          if(visible(button)&&!disabled&&button.getAttribute('aria-hidden')!=='true')return button;
        }
      }
    }
    return null;
  ...
  }  async function sendToChatGPT(prompt,attachments){const input=await waitFor(composer);if(!input)throw new Error('ChatGPT composer not found');input.focus({preventScroll:true});setText(input,prompt);if(attachments?.length)await attachFiles(input,attachments);phase='WAIT_SEND_READY';systemText.textContent='입력 완료 · 전송 버튼 감시 중';void monitorSendReady(prompt);} async function monitorSendReady(prompt){while(phase==='WAIT_SEND_READY'&&activeTaskId){const input=composer();if(input&&composerHasPrompt(input,prompt)){const send=sendButton();if(send){activateSendButton(send);phase='WAIT_RESPONSE';systemText.textContent='전송 요청 완료 · 응답 대기 중';return;}await new Promise(resolve=>setTimeout(resolve,250));}}}  function latestAssistant(){const nodes=[...document.querySelectorAll('[data-message-author-role="assistant"]')];return nodes.length?nodes[nodes.length-1].innerText.trim():'';}
  ...
  let stableTimer=0;
  function observeResponse(){if(phase!=='WAIT_RESPONSE'||!activeTaskId)return;const text=latestAssistant();if(!text||text===baselineAssistant)return;responseBlock.classList.remove('hidden');webResponse.textContent=text;clearTimeout(stableTimer);if(assistantStreaming())return;stableTimer=setTimeout(()=>{const now=latestAssistant();if(now===text&&!assistantStreaming())submitResult(now);},5000);}
  async function submitResult(text){if((phase!=='WAIT_RESPONSE'&&phase!=='WAIT_WORKER')||!activeTaskId)return;phase='WAIT_WORKER';pendingResult=text;setTask('CLAIMED',{id:activeTaskId,owner:'WEB',prompt:workerMessage.textContent,result:text});const body={success:true,taskId:activeTaskId,conversationId:currentConversationId,leaseId:activeLeaseId,responseText:text,resultType:'TEXT_RESULT'};let lastError=null;for(let attempt=0;attempt<3;attempt++){try{const r=await fetchWithTimeout(url('task/'+activeTaskId+'/result'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)},6000);const p=await r.json();if(!p.ok)throw new Error(p.data?.error||'result rejected');pendingResult=null;phase='FINISHED';setTask('COMPLETED',p.data);return;}catch(e){lastError=e;await new Promise(resolve=>setTimeout(resolve,500*(attempt+1)));}}systemText.textContent='Worker 결과 전달 실패: '+lastError.message+' · 재시도 대기';phase='WAIT_WORKER';}
  async function deliverClaimedTask(claimed){sentTaskId=claimed.id;activeLeaseId=claimed.leaseId||activeLeaseId||null;saveTaskMemory();setTask('PENDING',claimed);workerMessage.textContent=claimed.prompt;phase='WAIT_SEND_READY';systemText.textContent='Worker Message를 전달하고 있습니다.';try{await sendToChatGPT(claimed.prompt,claimed.attachments||[]);}catch(error){if(String(error.message||error).startsWith('WEB_REQUIRES_FOREGROUND')){phase='WAIT_FOREGROUND';systemText.textContent='WEB_REQUIRES_FOREGROUND: ChatGPT 탭을 활성화하면 전송을 재개합니다.';setTask('CLAIMED',claimed);return;}systemText.textContent='Worker Message 전달 실패: '+error.message;phase='IDLE';sentTaskId=null;activeLeaseId=null;await fetchWithTimeout(url('task/'+claimed.id+'/result'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({success:false,taskId:claimed.id,conversationId:currentConversationId,leaseId:claimed.leaseId,responseText:error.message,resultType:'TEXT_RESULT',finishReason:'send_failed'})});}}async function bind(){if(!currentConversationId)return;await fetchWithTimeout(url('bind'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({conversationId:currentConversationId,projectId:currentProjectId})});refresh();}
  async function refreshNow(autoBind=false){const refreshGeneration=resetGeneration;try{const previousConversationId=currentConversationId;const cid=conversation();currentConversationId=cid;const [sr,pr,tr,br]=await Promise.all([fetchWithTimeout(url('status')),fetchWithTimeout(url('projects')),fetchWithTimeout(url('task?conversationId='+(encodeURIComponent(cid||'__none__')))),cid?fetchWithTimeout(url('bindings/'+encodeURIComponent(cid))):Promise.resolve(null)]);if(!sr.ok||!pr.ok||!tr.ok)throw new Error('bridge unavailable');const status=await sr.json(),projects=await pr.json(),taskPayload=await tr.json(),binding=br?await br.json():null;if(refreshGeneration!==resetGeneration)return;const project=projects.data?.[0];let task=taskPayload.data?.task;if(task&&currentConversationId){const ignoredTaskId=sessionStorage.getItem('gptweb-hub-reset-task-'+currentConversationId);if(ignoredTaskId===task.id){task=null;}else if(ignoredTaskId){sessionStorage.removeItem('gptweb-hub-reset-task-'+currentConversationId);}}currentProjectId=project?.id||'';let bound=binding?.data?.bound?binding.data.projectId:'';if(autoBind&&cid&&lastBoundConversationId===previousConversationId){const sync=await fetchWithTimeout(url('bind'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({conversationId:cid,projectId:currentProjectId})});if(sync.ok)bound=currentProjectId;}lastBoundConversationId=bound?cid:null;if(sr.ok) await fetchWithTimeout(url('heartbeat'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({client:'gptweb-hub',conversationId:cid,projectId:currentProjectId,conversationTitle:title(),extensionVersion:EXTENSION_VERSION,extensionBuild:EXTENSION_BUILD})});if(refreshGeneration!==resetGeneration)return;setStatus('web',title(),cid?'ok':'pending');setStatus('worker',status.data?.repository||'—',status.data?.repository?'ok':'pending');setStatus('status','Connected','ok');systemText.textContent='정상적으로 연결되어 있습니다.';const connect=root.querySelector('.web-connect');connect.hidden=!cid||!!bound;connect.textContent=bound?'연결됨':'연결';if(!cid){phase='IDLE';setTask('IDLE');systemText.textContent='현재 ChatGPT 대화를 식별할 수 없습니다.';return;}if(!bound){phase='IDLE';setTask('IDLE');systemText.textContent='현재 GPT Web 대화를 Worker 프로젝트와 연결하세요.';return;}if(!task){if(phase!=='WAIT_RESPONSE'&&phase!=='WAIT_WORKER')phase='IDLE';if(phase==='IDLE')setTask('IDLE');systemText.textContent='정상적으로 연결되어 있습니다.';return;}const previousTaskId=activeTaskId;activeTaskId=task.id;if(previousTaskId&&previousTaskId!==task.id){phase='IDLE';sentTaskId=null;baselineAssistant='';}if(['COMPLETED','FAILED'].includes(task.status)){phase='FINISHED';setTask(task.status,task);return;}if(task.status==='PENDING'&&phase==='IDLE'&&!sentTaskId){baselineAssistant=latestAssistant();const claim=await fetchWithTimeout(url('task/'+task.id+'/claim'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({conversationId:currentConversationId})});const cp=await claim.json();if(cp.ok){await deliverClaimedTask(cp.data);}return;}if(task.status==='CLAIMED'){if(phase==='IDLE'){restoreTaskMemory(task.id);if(sentTaskId!==task.id||!baselineAssistant){systemText.textContent='기존 task의 응답 기준점을 복구할 수 없습니다. 중복 전송을 막기 위해 대기합니다.';setTask('CLAIMED',task);return;}await deliverClaimedTask(task);}else if(phase==='WAIT_FOREGROUND'){await deliverClaimedTask(task);}setTask('CLAIMED',task);if(!sentTaskId)systemText.textContent='이미 전달된 작업입니다. 응답을 기다립니다.';}}catch(e){setStatus('status','Disconnected','offline');systemText.textContent='Worker bridge에 연결할 수 없습니다.';}}
  ...
  root.querySelector('.settings').onclick=openSettings;root.querySelector('.settings-close').onclick=()=>settingsModal.classList.add('hidden');root.querySelector('.settings-cancel').onclick=()=>settingsModal.classList.add('hidden');root.querySelector('.web-connect').onclick=bind;root.querySelector('.close').onclick=()=>{panel.classList.add('hidden');root.querySelector('.reopen').classList.add('visible')};root.querySelector('.reopen').onclick=()=>{panel.classList.remove('hidden');root.querySelector('.reopen').classList.remove('visible')};
  ...
  let last=location.href;const nav=()=>{if(location.href===last)return;const followBinding=!!currentConversationId&&lastBoundConversationId===currentConversationId;last=location.href;activeTaskId=null;sentTaskId=null;activeLeaseId=null;baselineAssistant='';pendingResult=null;phase='IDLE';setTask('IDLE');refresh(followBinding)};const push=history.pushState;history.pushState=function(){const r=push.apply(this,arguments);nav();return r};const replace=history.replaceState;history.replaceState=function(){const r=replace.apply(this,arguments);nav();return r};addEventListener('popstate',nav);addEventListener('hashchange',nav);new MutationObserver(observeResponse).observe(document.body,{subtree:true,childList:true,characterData:true});loadSettings();setInterval(()=>refresh(),1500);
})();
