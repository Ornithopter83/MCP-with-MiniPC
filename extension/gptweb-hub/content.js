(() => {
  ...
  const EXTENSION_BUILD = '2026-09-25.1';
  ...
  let settings=Object.assign({},DEFAULT), currentConversationId=null, lastBoundConversationId=null, currentProjectId='', activeTaskId=null, sentTaskId=null, activeLeaseId=null, activeResource=null, phase='IDLE', baselineAssistant='', baselineAssistantElement=null, baselineAssistantKey='', baselineAssistantCount=-1, baselineUserMessages=[], baselineImageSources=new Set(), pendingResult=null, lastProgressKey='', activeTaskOwner=null, responseStarted=false, responseStartedAt=0, responseDeadlineTimer=0, lastImageProgressKey='', stableSnapshot='', progressQueue=Promise.resolve(); let resetGeneration=0, navigationGeneration=0; const DELIVERY_TIMEOUT=180000, COMPOSER_TIMEOUT=120000, SEND_CONFIRM_TIMEOUT=45000, RESOURCE_IMAGE_WAIT_TIMEOUT=120000, RESOURCE_IMAGE_SETTLE_DELAY=5000;
...
function responseSnapshot(){const images=activeResource?.type==='IMAGE'?latestGeneratedImageCandidates():[];return latestAssistant()+'|'+images.map(imageFingerprint).join('|');}
...
function resetResponseTracking(){clearResponseTimers();responseStarted=false;responseStartedAt=0;lastImageProgressKey='';}
function captureAssistantBaseline(){const nodes=assistantMessages();baselineAssistantElement=nodes.length?nodes[nodes.length-1]:null;baselineAssistant=baselineAssistantElement?.innerText.trim()||'';baselineAssistantKey=assistantMessageKey(baselineAssistantElement);baselineAssistantCount=nodes.length;captureImageBaseline();resetResponseTracking();}
...
let stableTimer=0;
function scheduleStableCheck(snapshot,delay,callback,allowStreaming=false){if(stableSnapshot===snapshot&&stableTimer)return;clearTimeout(stableTimer);stableSnapshot=snapshot;stableTimer=setTimeout(async()=>{stableTimer=0;if(phase!=='WAIT_RESPONSE'||!activeTaskId||responseSnapshot()!==snapshot||(!hasNewAssistantTurn()&&!latestGeneratedImageCandidates().length)||(!allowStreaming&&assistantStreaming()))return;await callback();},delay);}
function scheduleResponseDeadline(){if(responseDeadlineTimer||!activeTaskId||activeResource?.type!=='IMAGE')return;if(!responseStartedAt)responseStartedAt=Date.now();const remaining=Math.max(1000,RESOURCE_IMAGE_WAIT_TIMEOUT-(Date.now()-responseStartedAt));responseDeadlineTimer=setTimeout(async()=>{responseDeadlineTimer=0;if(phase!=='WAIT_RESPONSE'||!activeTaskId)return;const loaded=latestGeneratedImages();if(loaded.length){reportProgress('IMAGE_READY',loaded.length+'개 생성 이미지 확인 · deadline');await submitResult(hasNewAssistantTurn()?latestAssistant():('RESOURCE images generated: '+loaded.length));return;}const reason='RESOURCE_IMAGE_NOT_GENERATED: response deadline exceeded without downloadable images.\n'+(hasNewAssistantTurn()?latestAssistant():'');reportProgress('RESOURCE_NO_IMAGE','시간 내 다운로드 가능한 이미지를 확인하지 못했습니다.');await failTask(reason,'resource_image_not_generated');},remaining);}
function observeResponse(){if(phase!=='WAIT_RESPONSE'||!activeTaskId)return;const isImage=activeResource?.type==='IMAGE',candidates=isImage?latestGeneratedImageCandidates():[],newAssistant=hasNewAssistantTurn();if(!newAssistant&&!(isImage&&candidates.length))return;const text=newAssistant?latestAssistant():'',images=isImage?latestGeneratedImages():[];if(!text&&!candidates.length)return;responseBlock.classList.remove('hidden');webResponse.textContent=text||(candidates.length?'생성 이미지 처리 중':'응답을 기다리고 있습니다.');if(!responseStarted){responseStarted=true;responseStartedAt=Date.now();reportProgress('RESPONSE_START',isImage?'RESOURCE assistant/image 응답을 확인했습니다.':'새 assistant 응답을 확인했습니다.');if(isImage)scheduleResponseDeadline();}if(isImage){const progressKey=candidates.length+'/'+images.length;if(progressKey!==lastImageProgressKey){lastImageProgressKey=progressKey;reportProgress('IMAGE_DETECTED','candidate='+candidates.length+', loaded='+images.length);}for(const candidate of candidates){if(!candidate.complete&&!candidate.dataset.projecthubLoadWatch){candidate.dataset.projecthubLoadWatch='1';candidate.addEventListener('load',()=>{candidate.dataset.projecthubLoadWatch='';observeResponse();},{once:true});candidate.addEventListener('error',()=>{candidate.dataset.projecthubLoadWatch='';observeResponse();},{once:true});}}if(images.length){const snapshot=responseSnapshot(),delay=assistantStreaming()?15000:RESOURCE_IMAGE_SETTLE_DELAY;scheduleStableCheck(snapshot,delay,async()=>{const loaded=latestGeneratedImages();if(!loaded.length)return;reportProgress('IMAGE_READY',loaded.length+'개 생성 이미지 확인');await submitResult(hasNewAssistantTurn()?latestAssistant():('RESOURCE images generated: '+loaded.length));},true);}return;}if(assistantStreaming()){clearTimeout(stableTimer);stableTimer=0;stableSnapshot='';return;}const snapshot=responseSnapshot();scheduleStableCheck(snapshot,5000,async()=>{reportProgress('RESPONSE_STABLE','응답이 안정되어 결과를 전달합니다.');await submitResult(latestAssistant());});}
