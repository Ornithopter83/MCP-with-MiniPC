(() => {
  ...
  const EXTENSION_BUILD = '2026-09-23.5';
  ...
  let settings=Object.assign({},DEFAULT), currentConversationId=null, lastBoundConversationId=null, currentProjectId='', activeTaskId=null, sentTaskId=null, activeLeaseId=null, phase='IDLE', baselineAssistant='', baselineAssistantElement=null, baselineAssistantKey='', baselineAssistantCount=-1, baselineUserMessages=[], pendingResult=null, lastProgressKey=''; let resetGeneration=0, navigationGeneration=0; const DELIVERY_TIMEOUT=180000, COMPOSER_TIMEOUT=120000, SEND_CONFIRM_TIMEOUT=45000;
  ...
  function url(path,cfg){const c=cfg||settings;return 'http://'+c.bridgeHost+':'+c.bridgePort+c.bridgeBasePath+'/'+path;} function reportProgress(stage,detail='',attempt=0){if(!activeTaskId||!currentConversationId)return;const key=stage+'|'+detail+'|'+attempt;if(key===lastProgressKey)return;lastProgressKey=key;void fetchWithTimeout(url('progress'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({taskId:activeTaskId,conversationId:currentConversationId,leaseId:activeLeaseId||'',stage,detail,attempt})},8000).catch(()=>{});} function setPhase(next,detail='',attempt=0){phase=next;reportProgress(next,detail,attempt);}
