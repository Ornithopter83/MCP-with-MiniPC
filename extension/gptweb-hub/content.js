(() => {
  ...
  const EXTENSION_BUILD = '2026-09-24.1';
...
function latestGeneratedImage(){const assistant=latestAssistantElement();const local=(assistant?[...assistant.querySelectorAll('img')]:[]).filter(img=>!!(img.currentSrc||img.src));if(local.length)return local[local.length-1];const candidates=[...document.querySelectorAll('main img')].filter(img=>{const src=img.currentSrc||img.src||'';return src&&visible(img)&&(img.naturalWidth>=256||img.width>=256)&&(img.naturalHeight>=256||img.height>=256);});return candidates.length?candidates[candidates.length-1]:null;}
function responseSnapshot(){const img=activeResource?.type==='IMAGE'?latestGeneratedImage():null;return latestAssistant()+'|'+(img?.currentSrc||img?.src||'');}
async function imagePayload(){const image=latestGeneratedImage();if(!image)throw new Error('RESOURCE_IMAGE_NOT_FOUND');const src=image.currentSrc||image.src;if(!src)throw new Error('RESOURCE_IMAGE_SRC_MISSING');const response=await fetchWithTimeout(src,{credentials:'include'},120000);if(!response.ok)throw new Error('RESOURCE_IMAGE_DOWNLOAD_HTTP_'+response.status);const blob=await response.blob();if(!blob.size)throw new Error('RESOURCE_IMAGE_EMPTY');const buffer=new Uint8Array(await blob.arrayBuffer());let binary='';const chunk=0x8000;for(let i=0;i<buffer.length;i+=chunk)binary+=String.fromCharCode(...buffer.subarray(i,i+chunk));return {resultFileBase64:btoa(binary),resultFileMimeType:blob.type||'image/png',resultFileName:activeResource?.targetFileName||'resource.png'};}
...
function hasNewAssistantTurn(){const nodes=assistantMessages();if(!nodes.length)return false;if(baselineAssistantCount>=0&&nodes.length>baselineAssistantCount)return true;const latest=nodes[nodes.length-1],key=assistantMessageKey(latest);return (!!baselineAssistantElement&&latest!==baselineAssistantElement)||(!!baselineAssistantKey&&!!key&&key!==baselineAssistantKey);}
  ...
  let stableTimer=0;
  function observeResponse(){if(phase!=='WAIT_RESPONSE'||!activeTaskId)return;const text=latestAssistant(),image=activeResource?.type==='IMAGE'?latestGeneratedImage():null;if((!text&&!image)||!hasNewAssistantTurn())return;responseBlock.classList.remove('hidden');webResponse.textContent=text||(image?'생성 이미지 감지':'응답을 기다리고 있습니다.');reportProgress('RESPONSE_START',image?'새 assistant 이미지 응답을 확인했습니다.':'새 assistant 응답을 확인했습니다.');clearTimeout(stableTimer);if(assistantStreaming())return;const snapshot=responseSnapshot();reportProgress('RESPONSE_STABLE','응답 완료 여부를 확인하는 중');stableTimer=setTimeout(()=>{if(responseSnapshot()===snapshot&&hasNewAssistantTurn()&&!assistantStreaming())submitResult(latestAssistant()||(image?'RESOURCE image generated':''));},5000);}
  function resumeClaimedResponse(task){activeResource=task.resource||null;sentTaskId=task.id;activeLeaseId=task.leaseId||activeLeaseId||null;saveTaskMemory();setTask('PENDING',task);workerMessage.textContent=task.prompt;reportProgress('CLAIMED','전송된 대화의 응답 수신을 재개했습니다.');setPhase('WAIT_RESPONSE','기존 사용자 메시지를 확인해 응답 수신을 재개합니다.');systemText.textContent='ChatGPT 응답을 Worker로 전달하는 중입니다.';observeResponse();}
  async function submitResult(text){
    ...
    pendingResult=text;
    setTask('CLAIMED',{id:activeTaskId,owner:'WEB',prompt:workerMessage.textContent,result:text});
    ...
    if(activeResource?.type==='IMAGE'){
      try{Object.assign(body,await imagePayload());body.resultType='RESOURCE_IMAGE';}
      catch(error){await failTask('RESOURCE_IMAGE_CAPTURE: '+(error.message||error));return;}
    }
    let lastError=null;
