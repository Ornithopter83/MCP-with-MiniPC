...
function setStatus(kind,value,tone){const e=root.querySelector('.status-row[data-kind="'+kind+'"] .status-value');if(!e)return;e.textContent=value;e.className='status-value status-'+tone;}
  ...
  function promptConversationPosition(prompt){const turns=conversationTurnContainers();for(let index=turns.length-1;index>=0;index--){const turn=turns[index];if(!promptMatchesText(turn.text,prompt))continue;const fingerprint=containerFingerprint(turn);if(baselineContainerFingerprints.has(fingerprint))continue;return {turns,index,user:turn,response:turns.slice(index+1).find(candidate=>normalizeText(candidate.text))||null};}return null;}
...
function readyResponseFileCandidates(){return latestGeneratedFileCandidates().map(file=>({kind:'file',element:file.element,url:file.url,fileName:file.fileName||''}));}
