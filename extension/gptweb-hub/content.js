...
function setStatus(kind,value,tone){const e=root.querySelector('.status-row[data-kind="'+kind+'"] .status-value');if(!e)return;e.textContent=value;e.className='status-value status-'+tone;}
  ...
  function currentResponseText(prompt){const response=responseTurnAfterPrompt(prompt);if(response?.text)return normalizeText(response.text);return assistantTurnEvidence()?latestAssistant():'';}
...
function latestResponseFileCandidates(){const prompt=normalizeText(workerMessage?.textContent||''),response=responseTurnAfterPrompt(prompt),root=response?.element||latestAssistantElement();if(!root)return [];const result=[],seen=new Set();for(const item of collectDownloadableFiles(root)){if(!item.url||baselineFileUrls.has(item.url)||seen.has(item.url))continue;seen.add(item.url);result.push(item);}return result;}
...
function readyResponseFileCandidates(){return latestResponseFileCandidates().map(file=>({kind:'file',element:file.element,url:file.url,fileName:file.fileName||''}));}
...
function responseSnapshot(){const prompt=normalizeText(workerMessage?.textContent||'');const resources=activeResource?latestGeneratedResourceCandidates():readyResponseFileCandidates();return currentResponseText(prompt)+'|'+resources.map(resourceFingerprint).join('|');}
