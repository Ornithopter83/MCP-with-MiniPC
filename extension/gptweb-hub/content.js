(() => {
  ...
  const EXTENSION_BUILD = '2026-09-24.3';
...
function latestGeneratedImageCandidate(){const assistant=latestAssistantElement();if(!assistant)return null;const images=[...assistant.querySelectorAll('img')].filter(img=>!!(img.currentSrc||img.src));return images.length?images[images.length-1]:null;}
function latestGeneratedImage(){const image=latestGeneratedImageCandidate();return image&&visible(image)&&image.complete&&image.naturalWidth>=128&&image.naturalHeight>=128?image:null;}
...
function hasNewAssistantTurn(){const nodes=assistantMessages();if(!nodes.length)return false;if(baselineAssistantCount>=0&&nodes.length>baselineAssistantCount)return true;const latest=nodes[nodes.length-1],key=assistantMessageKey(latest);return (!!baselineAssistantElement&&latest!==baselineAssistantElement)||(!!baselineAssistantKey&&!!key&&key!==baselineAssistantKey);}
  ...
  let stableTimer=0;
  function scheduleStableCheck(snapshot,delay,callback){if(stableSnapshot===snapshot&&stableTimer)return;clearTimeout(stableTimer);stableSnapshot=snapshot;stableTimer=setTimeout(async()=>{stableTimer=0;if(phase!=='WAIT_RESPONSE'||!activeTaskId||responseSnapshot()!==snapshot||!hasNewAssistantTurn()||assistantStreaming())return;await callback();},delay);}
  function observeResponse(){if(phase!=='WAIT_RESPONSE'||!activeTaskId||!hasNewAssistantTurn())return;const isImage=activeResource?.type==='IMAGE',text=latestAssistant(),image=isImage?latestGeneratedImage():null,candidate=isImage?latestGeneratedImageCandidate():null;if(!text&&!candidate)return;responseBlock.classList.remove('hidden');webResponse.textContent=text||(candidate?'생성 이미지 처리 중':'응답을 기다리고 있습니다.');if(!responseStarted){responseStarted=true;reportProgress('RESPONSE_START',isImage?'RESOURCE assistant 응답을 확인했습니다.':'새 assistant 응답을 확인했습니다.');}if(assistantStreaming()){clearTimeout(stableTimer);stableTimer=0;stableSnapshot='';return;}if(isImage&&!image&&candidate&&!candidate.complete&&!candidate.dataset.projecthubLoadWatch){candidate.dataset.projecthubLoadWatch='1';candidate.addEventListener('load',()=>{candidate.dataset.projecthubLoadWatch='';observeResponse();},{once:true});candidate.addEventListener('error',()=>{candidate.dataset.projecthubLoadWatch='';observeResponse();},{once:true});}const snapshot=responseSnapshot();if(isImage&&!image){scheduleStableCheck(snapshot,RESOURCE_IMAGE_WAIT_TIMEOUT,async()=>{const loaded=latestGeneratedImage();if(loaded){reportProgress('RESPONSE_STABLE','생성 이미지 로드를 확인해 결과를 전달합니다.');await submitResult(latestAssistant()||'RESOURCE image generated');return;}const reason='RESOURCE_IMAGE_NOT_GENERATED: assistant 응답은 완료되었지만 생성 이미지를 확인하지 못했습니다.\n'+latestAssistant();reportProgress('RESOURCE_NO_IMAGE','assistant 응답에서 생성 이미지를 확인하지 못했습니다.');await failTask(reason,'resource_image_not_generated');});return;}scheduleStableCheck(snapshot,5000,async()=>{reportProgress('RESPONSE_STABLE','응답이 안정되어 결과를 전달합니다.');await submitResult(latestAssistant()||(image?'RESOURCE image generated':''));});}
  ...
  async function submitResult(text){
    ...
    if(activeResource?.type==='IMAGE'){
      ...
      catch(error){const message=String(error.message||error);const finishReason=/DOWNLOAD_HTTP|RESOURCE_IMAGE_EMPTY/i.test(message)?'resource_image_download_failed':'resource_image_capture_failed';await failTask('RESOURCE_IMAGE_CAPTURE: '+message,finishReason);return;}
    }
    let lastError=null;
