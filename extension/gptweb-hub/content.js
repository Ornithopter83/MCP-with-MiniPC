(() => {
  ...
  const EXTENSION_BUILD = '2026-09-24.2';
...
function latestGeneratedImage(){const assistant=latestAssistantElement();if(!assistant)return null;const images=[...assistant.querySelectorAll('img')].filter(img=>{const src=img.currentSrc||img.src||'';return src&&visible(img)&&img.complete&&img.naturalWidth>=128&&img.naturalHeight>=128;});return images.length?images[images.length-1]:null;}
...
function hasNewAssistantTurn(){const nodes=assistantMessages();if(!nodes.length)return false;if(baselineAssistantCount>=0&&nodes.length>baselineAssistantCount)return true;const latest=nodes[nodes.length-1],key=assistantMessageKey(latest);return (!!baselineAssistantElement&&latest!==baselineAssistantElement)||(!!baselineAssistantKey&&!!key&&key!==baselineAssistantKey);}
  ...
  function observeResponse(){if(phase!=='WAIT_RESPONSE'||!activeTaskId||!hasNewAssistantTurn())return;const isImage=activeResource?.type==='IMAGE',text=latestAssistant(),image=isImage?latestGeneratedImage():null;if(!text&&!image)return;responseBlock.classList.remove('hidden');webResponse.textContent=text||(image?'생성 이미지 감지':'응답을 기다리고 있습니다.');if(!responseStarted){responseStarted=true;reportProgress('RESPONSE_START',isImage?'RESOURCE assistant 응답을 확인했습니다.':'새 assistant 응답을 확인했습니다.');}if(assistantStreaming()){clearTimeout(stableTimer);stableTimer=0;stableSnapshot='';return;}const snapshot=responseSnapshot();if(isImage&&!image){scheduleStableCheck(snapshot,30000,async()=>{if(latestGeneratedImage())return;const reason='RESOURCE_IMAGE_NOT_GENERATED: assistant 응답은 완료되었지만 생성 이미지가 없습니다.\n'+latestAssistant();reportProgress('RESOURCE_NO_IMAGE','assistant 응답에 생성 이미지가 없습니다.');await failTask(reason,'resource_image_not_generated');});return;}scheduleStableCheck(snapshot,5000,async()=>{reportProgress('RESPONSE_STABLE','응답이 안정되어 결과를 전달합니다.');await submitResult(latestAssistant()||(image?'RESOURCE image generated':''));});}
