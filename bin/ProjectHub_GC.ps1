...
if ($Apply) { foreach ($session in $safe) { $body=@{projectId=$session.projectId;workstationId=$session.workstationId;objectHash=$session.object.sha256;sizeBytes=$session.object.sizeBytes;operation=3;uploadSessionId=$session.sessionId;storageScope=$session.storageScope}|ConvertTo-Json; $token=(Invoke-RestMethod ($ServerBaseUrl.TrimEnd('/')+'/api/large-data/assertions') -Method Post -ContentType 'application/json' -Body $body -TimeoutSec 30).assertion; $cleanup=Invoke-RestMethod ($GatewayUrl.TrimEnd('/')+'/cleanup-session.php') -Method Post -Headers @{Authorization="Bearer $token"} -TimeoutSec 30; if ($cleanup.state -eq 'cleaned') { Write-Host "[DELETED] $($session.sessionId)" -ForegroundColor Green } elseif ($cleanup.state -eq 'already_clean') { Write-Host "[ALREADY_CLEAN] $($session.sessionId)" -ForegroundColor DarkGreen } else { throw "Unexpected cleanup response for $($session.sessionId)" } } }
