...
- 제공자/model descriptor와 OpenAI 목록
- 기존 JSON 제공자 문자열 호환
...
- Claude/Muse: 공식 symbol asset이 준비될 때까지 current-console color/gray 대체
...
- coordinator-first HQ/WORK/HIGH 실행은 등록소를 경유
- Claude/Muse 선택 시 `CLAUDE_NOT_CONFIGURED` / `MUSE_NOT_CONFIGURED`
- 미연결 제공자를 OpenAI로 대체하지 않음

### 사전 점검 / 마이그레이션

- OpenAI 제공자는 Codex CLI transport/auth/model/reasoning을 검증
- HIGH permit이 있을 때만 HIGH 제공자 preflight 포함
