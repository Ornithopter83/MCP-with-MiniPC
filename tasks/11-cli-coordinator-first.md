...
INSTRUCTION, REPORT, VALIDATION REQUEST, JUDGMENT 같은 의미 구역 표식는 신규 CLI 계약에서 사용하지 않는다.
...
- WORK의 GOTO:JUDGE 뒤 body 전체를 제공자 request 출처로 사용
- 기본 JEV raw response 앞에 JUDGMENT 표식를 붙이지 않음
- same WORK 세션에 raw body를 그대로 전달
...
CreateHistoryEvent()에서 신규 CLI role을 LUNA/JEV/SOL/HIGH 같은 출처 문자열로 역추론하지 않는다.
...
- 현재 CodexCliFile(path/name/mime/size)만으로 변경 유형을 만들지 않음
...
- native JUDGE raw body가 표식 없이 same WORK 세션으로 복귀
- WORK response completion 시 작업 이력 card 1개 생성
- HIGH response completion 시 고수준 이력 card 1개 생성
- JUDGE response completion 시 Judge 이력 card 1개 생성
- HQ response completion 시 관제 이력 card 생성
...
적용:
- HQ/WORK/HIGH/JUDGE 역할 하단 계약에서 semantic body tag 요구 제거
- JudgeTransportContract의 VALIDATION REQUEST 표식 의존 제거
- native JUDGE raw response의 JUDGMENT 표식 재삽입 제거
- HQ/WORK/JUDGE/HIGH/UNKNOWN 응답 완료 지점에서 typed 이력 카드 기록
- 이력 카드 1줄 미리보기 + token line + file line UI 적용
...
11-C-GOTO-CONTRACT는 ACTION/GOTO 외 semantic body tag 의존성이 신규 CLI 실행 시점과 이력에서 제거되고, Explorer에서 역할별 카드가 누락 없이 표시된 뒤 완료 처리한다.
...
- WORK JEV footer와 API 문서에 atomic NOUL/SCORE/CHOICE, QID, criteria, 선택 PASS/SCOPE/COUNTEREXAMPLE/EVIDENCE, 일부 QID만 재판정 규칙을 추가했다. JUDGE 비활성 시 지침은 제거된다.
- 전체 테스트 69개 통과 (Worker 64, Agent 3, Core 1, Server 1), Release 빌드 경고 0/오류 0, publish/복사 성공. SHA-256 `8C22F3D50CFFEAB2E27A6D84C7568D84A9ECEEA04F5D7DEA9A5B70626891A95A`. Explorer 오류 복귀 E2E 잔여.
...
## 2026-09-24 UI 피드백 — current pipeline 활성 외곽선
...
- Routing, JEV, UNKNOWN, HIGH permit, 이력 동작은 변경하지 않았다.
