...
구현 완료(2026-09-17): `ProjectHub_Setup.cmd` 단일 진입점, `.projecthub/project.json`, 기존 문서 조건부 생성, `bin` 배포 디렉터리와 프로젝트별 Sync/Restore 런처, checkpoint 조회 API, NAS download endpoint. Restore는 별도 디렉터리에 복원하고 파일 크기와 SHA-256을 검증한다.
