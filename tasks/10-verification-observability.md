...
- ENGINE_HEADLESS, UI_BROWSER, HUMAN_UX, JEV의 기능/상태/실행기/범위를 별도로 기록한다.
...
- Codex/JEV의 작업·회차·역할·모델·추론·목적, 토큰 사용량, 프롬프트/하단 계약/근거/데이터 묶음 바이트, 지연 시간, 재시도 및 사용량 확인 여부을 호출 단위로 보존한다.
...
- JEV 응답 사용량이 없으면 usage_known=false, 토큰 필드는 null로 남긴다. GPT Web 사용량은 항상 UNKNOWN으로 기록한다.
...
2026-09-23 10-A/B/C: 봉투 구조에 ENGINE_HEADLESS/UI_BROWSER/HUMAN_UX/JEV 기능·상태를 추가했다. 호출별 Codex/JEV/Web 사용량, 데이터 묶음 크기·요약 해시·지연을 로컬 JSONL에 보존하고 사용량 미제공은 UNKNOWN으로 구분한다. Codex 사용량 중복 집계를 누적/증분 규칙으로 수정했다. JEV 음성 대조 6건 픽스처를 추가했다. 검증: `dotnet test ProjectHub.sln --configuration Debug --no-restore` 통과 (Core 1, Agent 3, Server 1, Worker 16), `git diff --check`, Release 게시 성공. `C:\AI-AGENT\Worker`/`C:\GameProject` 복사본 SHA-256 모두 `CCD2C0B67FC85652B5387D7E54D3FD69A337D7DFD9BAE21768958866C3868E55`이며 Bridge 준비 완료, 확장 동기화됨=true다. 실 TypeSafe 벤치마크는 호출/비용 측정하지 않았으므로 판별 성공률은 미측정이다.
