...
## 2026-09-24 12-B Provider-ready UI + runner boundary

Provider 구조의 다음 안전 단위를 구현했다.

- HQ / WORK / HIGH에 독립 Provider 선택 구조
- Provider 선택에 따라 model/reasoning catalog를 다시 바인딩
- HIGH가 WORK Provider를 재사용하던 저장 오류 제거
- OpenAI는 기존 Codex 모델/reasoning catalog를 그대로 사용
- Claude/Muse는 Provider 선택과 visual/runner 구조만 노출하고 실제 모델 catalog가 없으므로 model/reasoning UI를 미연결 상태로 비활성
- `ProviderVisualCatalog`로 설정창/상단 pipeline/신규 role History의 provider icon 경로 통일
- Claude/Muse 공식 자산 전에는 `current-console(.gray).png`를 중립 fallback으로 사용
- `IAiRoleRunner`, `AiRoleRunnerRegistry`, `OpenAiCodexRoleRunner` 도입
- coordinator-first HQ/WORK/HIGH 실제 실행은 runner registry를 경유
- Claude/Muse는 `UnconfiguredAiRoleRunner`로 `*_NOT_CONFIGURED`를 반환하고 OpenAI fallback 금지
- session capability를 Provider descriptor로 관리; 미연결 Provider의 thread selector 비활성
- CLI_TO_CLI의 과거 OpenAI coordinator `transport=web` 설정은 runtime normalize에서 `codex_cli`로 마이그레이션
- HIGH permit이 없으면 미연결 HIGH Provider가 일반 HQ/WORK 실행을 막지 않고, permit이 있으면 preflight에서 차단
- 기존 GOTO/JUDGE/UNKNOWN/HIGH one-shot 의미는 변경하지 않음
