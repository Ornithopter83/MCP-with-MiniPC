...
foreach ($name in 'ProjectHub_Sync.ps1','ProjectHub_LargeData_Uploader.ps1','ProjectHub_Restore.ps1','ProjectHub_Commit_Push.ps1','ProjectHub_Fetch_Pull.ps1') { $source=Join-Path $ProjectHubSource $name; $target=Join-Path $binDir $name; if (-not (Test-Path $target)) { if (Test-Path $source) { Copy-Item -LiteralPath $source -Destination $target } else { Invoke-WebRequest ($TemplateBaseUrl.TrimEnd('/')+'/'+$name) -OutFile $target -TimeoutSec 60 } } }
